	...
	void OnGUI(){
		...
		if(showQuest){
			...
			if(currentQuestShowing){
				...
				if(QuestHavedCheck(currentQuestShowing)){
					if(currentQuestShowing.IsSuccess){
						text = currentQuestShowing.FinishText;
						if(GUI.Button(new Rect(10,160,80,25),"Complete")){
							QuestCompleteCheck(currentQuestShowing);
							showQuest = false;
						}
					}else{
					...
					if(GUI.Button(new Rect(10,160,80,25),"Accept")){
						AddQuest(currentQuestShowing);
						showQuest = false;
					}
				}
				...
				GUI.TextArea(new Rect(10,30,280,120),text);
			}
			GUI.EndGroup();
		}
	}
